...
const AuthProvider = ({ children }) => {
    ...
    await userApi
      ...
      .then((response) => {
        ...
        localStorage.setItem(
          "user",
          JSON.stringify({ ...response.data, roles: normalizedRoles })
        );
      })
      .catch(async (error) => {
  ...
  const refreshAuthToken = async () => {
    ...
    await authApi
      .createTokenByRefreshToken({ token: refreshToken })
      .then(async (response) => {
        const { token, refreshToken } = response.data;
        saveTokens(token, refreshToken);
        await initializeUser(token);
  ...
  return (
    <AuthContext.Provider
      value={{ user, login, logout, register, getAuthToken, refreshAuthToken }}
    >
      {children}
    </AuthContext.Provider>
  );
